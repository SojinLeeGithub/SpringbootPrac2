package com.lsj.springbasic.filter;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // 3. 'Authorization' 필드 값에서 token 추출한다.
    private String parseBearerToken(HttpServletRequest request) {
                   ...
                   //  2. 'Authorization' 필드 값이 Bearer 형식인지 확인
                   // 'Authorization' 필드 값의 존재 여부 부터 확인
                   boolean hasAuthorization = StringUtils.hasText(authorization);
                   if (!hasAuthorization) return null;

                   // startsWith: 문자열이 "Bearer " 로 시작하는지 여부 판별해준다.
                   boolean isBearedr = authorization.startsWith("Bearer ");
                    if(!isBearedr) return null;
